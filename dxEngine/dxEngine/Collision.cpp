...
bool Collision::CheckSphere2Sphere(
    const Col_Sphere& sphere1,
    const Col_Sphere& sphere2,
    DirectX::XMVECTOR* inter
){
    //球体の中心と球体の中心の距離
    XMVECTOR distSph2Sph = sphere1.center - sphere2.center;
    float dist = XMVector3Length(distSph2Sph).m128_f32[0];
    //2つの球体の半径の合計より大きければ当たってない
    float sum = sphere1.radius + sphere2.radius;
    if (fabsf(dist) > sum)return false;

    //疑似交点を計算
    if (inter) {
        *inter = XMVector3Normalize(distSph2Sph) * sphere1.radius;
    }
}
...
bool Collision::CheckSphere2Plane(
    ...
    //座標系の原点から球の中心座標への距離
    XMVECTOR distV = XMVector3Dot(
        sphere.center, plane.normal
    );
    //平面の原点距離を減算で、平面と球の中心との距離が出る
...
void Collision::ClosestPtPoint2Triangle(
    ...
    //pointがp0_p2の辺領域の中にあるかどうかチェックし、
    //あればpointのp0_p2上に対する射影を返す
...
bool Collision::CheckLay2Triangle(
    const Col_Lay& lay,
    const Col_Triangle& triangle,
    float* distance,
    DirectX::XMVECTOR* inter
    ...
    if (XMVector3Dot(
        m,
        triangle.normal
    ).m128_f32[0] < -epsilon)return false;
...
bool Collision::CheckLay2Sphere(
    const Col_Lay& lay,
    const Col_Sphere& sphere,
    float* distance,
    DirectX::XMVECTOR* inter
    ...
    float b = XMVector3Dot(
        m,
        lay.dir
    ).m128_f32[0];
    float c = XMVector3Dot(
        m,
        m
    ).m128_f32[0] - sphere.radius * sphere.radius;
    //レイの視点が球の外側にあり(c > 0)、
    //レイが球から離れていく方向を指している場合(b > 0)、
    //当たらない
